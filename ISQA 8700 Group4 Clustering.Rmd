...
cleanData <- dat
cleanDataClustering <- subset(dat, select = c(HeartDisease, PhysicalHealth, MentalHealth))
#cleanDataClustering <- subset(dat, select = -c(Race, SkinCancer, KidneyDisease, Asthma, Diabetic, DiffWalking, AlcoholDrinking, BMI, GenHealth, PhysicalActivity, AgeCategory, Sex, Stroke, Smoking, SleepTime))
...
table(clusteringTrainingSet$HeartDisease, clusters$cluster)
```
...
table(clusteringTestingSet$HeartDisease, clusteringTestingSet$modelOutcome)
```
